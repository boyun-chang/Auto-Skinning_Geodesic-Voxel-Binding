...
string res, objPath, hierarchyPath, transformPath, outputCsvPath;
Mesh *_mesh;
...
void main(int argc, char **argv)
{
	...
	//_mesh = new Mesh("obj\\character_mesh.obj");
	if (argc >= 6)
	{
		_res = atoi(argv[1]);
		if (_res < 1) _res = 64;
		hierarchyPath = argv[2];
		transformPath = argv[3];
		objPath = argv[4];
		outputCsvPath = argv[5];

		_mesh = new Mesh(_res, objPath.c_str(), hierarchyPath.c_str(), transformPath.c_str(), outputCsvPath.c_str());
	}
	else
	{
	...
	if (argc >= 5)
	{
		cout << "[C++] Processing finished. Exiting..." << endl;
		exit(0);
	}
	glutMainLoop();
}