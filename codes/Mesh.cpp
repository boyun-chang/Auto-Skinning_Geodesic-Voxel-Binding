...
void Mesh::loadObj(const char *file)
{
	...
	_gridMesh = new GridMesh(this, _res);
	_gridMesh->extractSkeleton();
	std::chrono::system_clock::time_point start = std::chrono::system_clock::now();
	_gridMesh->findPath(_outputCsvFile);
...
void Mesh::moveToCenter(Vec3<double> minBound, Vec3<double> maxBound, double scale)
{
	...
	//_skeleton = new Skeleton("data\\skeleton_transforms_5.txt", "data\\skeleton_hierarchy.txt", longestLength, center);
	_skeleton = new Skeleton(_transformFile, _hierarchyFile, longestLength, center);
